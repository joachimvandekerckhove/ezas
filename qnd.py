...
from base import (SummaryStats, Parameters, 
                   inverse_equations, forward_equations, resample_summary_stats)
...
def qnd_parameter_estimation(
    observed_summary_statistics: SummaryStats, 
    sample_size: int,
    ...
    Args:
        observed_summary_statistics: Observed summary statistics (SummaryStats)
    ...
    """
    if not isinstance(observed_summary_statistics, SummaryStats):
        raise TypeError("observed_summary_statistics must be a SummaryStats")
    if not isinstance(sample_size, int):
        raise TypeError("sample_size must be an integer")
        
    # Initialize the parameter matrix (K x 1 list of Parameters)
    parameter_matrix = [
        inverse_equations(resample_summary_stats(observed_summary_statistics, sample_size))
...
"""
def demonstrate_qnd_parameter_estimation(true_parameters: Parameters, sample_size: int):
    """Demonstrate basic QND parameter estimation.
    ...
    predicted_summary_statistics = forward_equations(true_parameters)
    observed_summary_statistics = resample_summary_stats(predicted_summary_statistics, sample_size)
    
    # Estimate the parameters and time the operation
    start_time = time.time()
    (qnd_parameters_mean, qnd_parameters_std, lower_quantile_params, upper_quantile_params) = qnd_parameter_estimation(observed_summary_statistics, sample_size, n_repetitions=1000)
    end_time = time.time()
...
"""
def qnd_parameter_estimation_beta_weights(
    observed_summary_statistics: list[SummaryStats],
    ...
    local_matrix.set_boundary([s.boundary for s in observed_summary_statistics])
    local_matrix.set_drift([s.drift for s in observed_summary_statistics])
    local_matrix.set_ndt([s.ndt for s in observed_summary_statistics])
...
def qnd_parameter_estimation_design_matrix(
    observed_summary_statistics: list[SummaryStats], 
    ...
    Returns:
        ...
        - upper_quantile_params: upper quantile of Parameters (as a Parameters object)
    """
    if not isinstance(observed_summary_statistics, list):
        raise TypeError("observed_summary_statistics must be a list of SummaryStats")
    if not isinstance(sample_size, list):
        raise TypeError("sample_size must be a list of integers")
...
Simulation study for basic QND parameter estimation
"""
def qnd_simulation_study(true_parameters: Parameters, sample_size: int, simulation_repetitions: int):
...
if __name__ == "__main__":
    ...
    if args.demo:
        print("Running demo...")
        demonstrate_qnd_parameter_estimation(true_parameters = Parameters(1.0, 0.5, 0.2), sample_size = 10)
        demonstrate_qnd_parameter_estimation(true_parameters = Parameters(1.0, 0.5, 0.2), sample_size = 160)
        
    if args.simulation_basic:
        print("Running simulation...")
        qnd_simulation_study(true_parameters = Parameters(1.0, 0.5, 0.2), sample_size = 100, simulation_repetitions = 1000)

    # if args.simulation_design_matrix:
    #     print("Running simulation with design matrix...")
    #     qnd_simulation_study_design_matrix(true_parameters = ez.Parameters(1.0, 0.5, 0.2), sample_size = 100, simulation_repetitions = 10000)
...
def qnd_design_matrix_parameter_estimation(
    ...
    """
    if not (isinstance(obs_stats, list) and all(isinstance(x, SummaryStats) for x in obs_stats)):
        raise TypeError("obs_stats must be a list of SummaryStats")
    if not (isinstance(N, list) and all(isinstance(x, int) for x in N)):
        raise TypeError("N must be a list of integers")
    ...
    qnd_params = inverse_equations(obs_stats)
    
    # Initialize the parameter matrix
    parameter_list = []
    for _ in range(1000):
        resampled_obs_stats = resample_summary_stats(obs_stats, N)
        resampled_qnd_params = inverse_equations(resampled_obs_stats)
